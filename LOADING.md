...
entrance paths, card media (skeleton → image/video), the reveal backfill, and
the indeterminate loading bar.
...
path. That is precisely what broke on 2026-08-03 (§7).
...
- **Video** (`applyCardMeta`): the element is created in JS, so there is no
  equivalent synchronous "already loaded" signal — see §7. Cache hits are
...
## 6. The indeterminate loading bar

One shared component, rendered by **two different hosts**, driven entirely by
two classes on the outer track (`.progress-bar-track`):

| Host | Where | `.is-loading` |
|---|---|---|
| `#preloader-progress-bar` | `preloader.hbs` — homepage first load | **hardcoded in markup** |
| `.scroll-progress` | `default.hbs` — every page, during navigation | toggled by `page-transition.js` |

The two hosts differ for a reason: the preloader element exists *only* while
loading, so it has no "off" state and the class is static. `.scroll-progress`
is the **same element as the real scroll-position indicator**, so it must be
switched in and out of the loading state — and that dual role is the source of
this component's subtlest bug (below).

### Motion: two one-shot states, never a loop

```
.is-loading   bar-grow        scaleX 0 → 1 from the left, then HOLDS (forwards)
.is-complete  bar-exit-right  translateX 0 → 100%, opacity fading from the 50% mark
```

- **Not `infinite`.** An earlier version looped, and each restart snapped from
  the end state back to `0%` — which read as the bar "going backwards". No
  amount of re-easing fixes that; the loop itself is the defect.
- **`forwards` on the grow is load-bearing.** Without it the bar snaps back to
  `scaleX(0)` the moment the grow ends, which looks like another reverse.
- **The exit is a translate, not a shrink.** Under `.progress-bar-clip`'s
  `overflow: hidden` these look identical — the visible left edge marches right
  while the right edge stays flush — but translate is compositor-only and keeps
  the gradient's soft ends from being squashed.
- **The exit's timing function is `linear` deliberately**, so "fade from 50%"
  is unambiguous: under linear interpolation 50% of elapsed time *is* 50% of
  distance travelled. Under any eased curve those diverge.

### Timing constants that must stay in sync

| Constant | Where | Must satisfy |
|---|---|---|
| `bar-grow` duration | `main.css` | — |
| `window.__BAR_CYCLE_MS` | `preloader.js` | **≥ `bar-grow` duration.** This gate is the only thing guaranteeing the one-shot grow has finished before `.is-complete` lands; the exit assumes a settled `scaleX(1)`. Set it lower and a fast load cuts the grow off mid-way. |
| class-strip `setTimeout` | `page-transition.js` | ≥ `bar-exit-right` duration, or the slide is cut off and the bar vanishes instead of leaving. |

### ⚠ The dual-role trap

`.scroll-progress`'s **base** rule is `width: 0%` + `transition: width 0.1s
linear` + a visible gradient, because it is also the scroll indicator. The
loading state overrides `width: 100% !important`.

So removing `.is-loading` reverts the **track** `100% → 0%`, and that
transition animates the collapse — a visible bar shrinking **right-to-left**,
immediately after the exit correctly travelled left-to-right. This is not
fixable from CSS: once the class is gone the element stops matching any
loading-scoped selector, so the base transition applies. It is suppressed
inline in `releaseLoadingBar()` (`page-transition.js`) around the removal:
...
**Anything that changes what `.scroll-progress` reverts to must re-check this.**
Full write-up in `COMMON_ISSUES.md` → "Loading Bar Goes Back Right-to-Left".

### Reduced motion

Both states are cancelled and pinned to their end values. Cancelling an
animation without restating its properties strands the element wherever the
animation last computed — for this component, an invisible or half-scaled bar.
The blurred `.progress-bar-glow` layer is pinned too; leaving it pulsing while
the streak is frozen is a bug this component has already produced once.

---

## 7. Fixed issues (keep these from regressing)
