...
// SCROLL LOCK REMOVED (2026-08-08).
//
// A 500ms lock used to run here on every page load: it forced scrollTo(0,0)
// immediately, then a 'scroll' listener reverted the page to 0 on EVERY
// scroll event until a timer released it. Every wheel/touch input in that
// window was discarded — measured maxY of 0 for the first 600ms on home,
// about and post pages alike. Combined with `html { scroll-behavior: smooth }`
// (main.css) each forced revert glided rather than snapped, so it read as
// the page jerking and fighting you rather than simply not moving.
//
// It was not earning that cost:
//   - Its stated job, "prevent the browser auto-restoring scroll position",
//     is already done by history.scrollRestoration = 'manual' directly above.
//   - Its other stated job, "prevent unwanted scroll-to-hash", guards against
//     something this site does not have: there are no same-page anchor links
//     in any template (grepped every .hbs for href="#section" — only Ghost
//     Portal's #/portal/* modal links, which never scroll).
//
// Hash handling for EXTERNAL links (someone opening /#helix directly) is kept
// below, just without the lock in front of it. The curtain-return carve-out
// that used to live here went with the lock — that flow restores a saved
// scroll position on load and only needed the exemption because the lock
// would have fought it.
if (window.location.hash) {
  const id = window.location.hash.slice(1);
  const element = document.getElementById(id);
  // Deferred a frame: at this point in main.js the target may not be laid out
  // yet, and scrollIntoView against an unpositioned element lands wrong.
  if (element) {
    setTimeout(() => {
...
function initPostHeaderAnimation() {
  ...
  const runHeaderAnimation = () => {
    ...
    tl.to(headerEl, {
      ...
      duration: 0.8,
      ease: 'power2.out',
      // GSAP leaves `filter` at its final tweened value (blur(0px)) instead
      // of clearing the property — blur(0px) is a LIVE filter, not "no
      // filter", so the browser keeps this element (and everything nested
      // inside it, including the post's feature <video>, which repaints
      // every frame while playing) on its own filter-effect compositing
      // layer permanently after the entrance animation is long done. Found
      // investigating a post-page-wide scroll flicker: Paint flashing
      // showed green across the ENTIRE page, not just the video's own box —
      // consistent with a filtered ancestor forcing the compositor to keep
      // recomposing everything under it.
      //
      // clearProps: 'filter' alone is NOT enough. It removes the inline
      // style GSAP set, but .post > .post-header's own stylesheet rule
      // (main.css) declares `filter: blur(4px)` as this element's resting
      // state — the entrance animation's pre-reveal flash guard — so
      // clearing the inline value just exposes that blur again underneath
      // it. Traced live: a gsap.set() call log showed clearProps firing
      // correctly, yet polling computed `filter` a moment later still read
      // blur(4px) — the inline override was gone, the stylesheet default
      // was still there. Setting `filter: 'none'` explicitly (inline) wins
      // over that stylesheet rule regardless, same as the fix already
      // applied to .post-header .post-image img/video for the identical
      // reason.
      onComplete: () => gsap.set(headerEl, { filter: 'none' }),
    }, 0);
    ...
    fetch(url)
      ...
      .then(html => {
        ...
        try {
          ...
          // Apply avatar (60px round headshot next to the snippet/source —
          // same codeinjection_head field convention as the text fields
          // here). Files live under content/images/avatars/ (confirmed
          // live: "paul.jpeg" resolves to content/images/avatars/paul.jpeg,
          // matching post-card.hbs's own videos/ subfolder convention) — an
          // absolute URL is used as-is instead.
          if (meta.avatar) {
            const avatarEl = card.querySelector('.testimonial-avatar');
            if (avatarEl) {
              avatarEl.src = meta.avatar.startsWith('http') ? meta.avatar : `/content/images/avatars/${meta.avatar}`;
              avatarEl.style.display = 'block';
              // console.log(`[testimonial] ✓ Applied avatar to card`);
            }
          }

          // Apply source (regular text underneath). innerHTML, not
          // textContent: source can carry a <br /> (e.g. "Name,<br />Title
          // at Company") to force the role onto its own line — textContent
          // would render that tag as literal text instead of a break.
          if (meta.source) {
...
// ═══════════════════════════════════════════════════════════════
// SPOTLIGHT GLOW — mouse-tracking radial gradient on [data-glow] panes
// ═══════════════════════════════════════════════════════════════

// One delegated mousemove listener for every [data-glow] pane on the page,
// same pattern as initTooltipSystem() just above rather than a listener
// per element — .content-grid-item/.field-grid-item are hand-typed into
// Ghost post content (no template renders them, no fixed count), so this
// has to work for however many exist without per-post JS wiring. Writes
// --glow-x/--glow-y (main.css's [data-glow]::before reads them) as the
// cursor's position RELATIVE TO the hovered pane's own box — the CSS mask
// this drives is `at var(--glow-x) var(--glow-y)`, which is relative to
// the masked element (::before, inset:0 on the pane), not the viewport.
function initSpotlightGlow() {
  if (window.matchMedia('(prefers-reduced-motion: reduce)').matches) return;

  document.addEventListener('mousemove', (e) => {
    if (!(e.target instanceof Element)) return;
    const pane = e.target.closest('[data-glow]');
    if (!pane) return;
    const rect = pane.getBoundingClientRect();
    pane.style.setProperty('--glow-x', `${e.clientX - rect.left}px`);
    pane.style.setProperty('--glow-y', `${e.clientY - rect.top}px`);
  }, { passive: true });
}
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  // initCustomScrollbar() removed — see ui-utilities.js for why.
  ...
  // initTestimonialModal() DISABLED (2026-08-10) — temporarily, per explicit
  // request: testimonial cards should not open their link/modal right now.
  // May come back, so the function itself is left intact (modal-data.js) —
  // uncomment this one call to restore. Nothing else needed reverting: the
  // card's own cursor:pointer and click listener are only ever attached
  // inside this function, so skipping the call is a real no-op, not a
  // partial disable.
