...
function applyCardMeta(card, rawText, onSettled) {
  ...
  try {
    ...
    if (meta.result) {
      // Same field post.hbs's own #project-result reads (detail page) —
      // reusing it here rather than a second card-only field means an
      // existing post that already sets "result" (gala-defi, i-genie,
      // snoop-dogg-foundation) shows it on the card with no extra
      // authoring. innerHTML, not textContent, matching post.hbs's own
      // handling of this exact field — result can carry real markup
      // there (not currently used by any live post, but the contract
      // should match on both surfaces rather than silently diverge).
    ...
    if (meta.cardKeywords) {
      ...
      // also used by posts-tabs-grid.js's identical .grid-card-keywords case.
      // Format doc (colon-suffixed color per keyword) lives there.
      window.renderCardKeywords(card.querySelector('.post-card-keywords'), meta.cardKeywords);
    }
    ...
    // data-tooltip off whatever's under the cursor). meta.tooltipLabel is a
    // NEW codeinjection_head field: unset → the default text below
    // (enabled/disabled read); set to real text → that text, either state;
    // set to '' (explicitly blank) → no tooltip at all, on purpose (a
    // per-post opt-out, not a missing-field case — hasOwnProperty, not a
    // falsy check, so "" only wins over the default when the field was
    // actually written, not just absent).
    {
      const link = card.querySelector('.post-card-link');
      if (link) {
...
// Plays/pauses a <video class="post-card-video"> pasted directly into a
// post's own body content (an HTML card in the editor, copied from a real
// card's rendered markup — post-card.hbs's own <video> has no autoplay
// attribute by design, relying entirely on showImageFallback() below to
// call .play(); that function only ever runs against a real .post-card
// element post-and-cards.js itself builds/populates, never against
// arbitrary HTML pasted into {{content}}). Reported live: video renders
// correctly (right source, right frame) but never starts, because nothing
// on the post's own page ever calls .play() on it — the card-grid path
// that normally does this never runs here at all, there is no .post-card
// ancestor for it to run against.
//
// Excludes any video already inside a real .post-card so this never
// double-wires the SAME element showImageFallback() already owns, if a
// real card ever renders its video into .gh-content some other way
// (defensive; no known path does that today, but this is exactly the
// class of bug free-standing markup-in-content invites).
//
// threshold 0.1, not showImageFallback()'s own 0.5: that card-grid
// observer can assume a modestly-sized, fixed-aspect card image — pasted
// post content has no such guarantee (measured live: a video pasted at
// content width can render TALLER than the whole viewport), and 50% of a
// too-tall element's own area may never be reachable at any scroll
// position. 0.1 (any meaningful sliver on screen) is what actually
// guarantees play/pause fires regardless of the element's own size
// relative to the viewport — confirmed live: 0.5 never fired at all for
// a 900px-tall test video in an 813px viewport, 0.1 fired immediately.
function initStandaloneCardVideos() {
  // .closest('.post-card'), not a :not() selector: :not() only accepts
  // simple/compound selectors, not "ancestor is X" — there is no valid
  // single CSS selector for "this element is NOT inside that one".
  const videos = Array.from(document.querySelectorAll('.gh-content .post-card-video'))
    .filter((video) => !video.closest('.post-card'));
...
if (document.readyState === 'loading') {
  ...
  document.addEventListener('DOMContentLoaded', initStandaloneCardVideos, { once: true });
} else {
  eagerFetchAllCardMeta();
  initStandaloneCardVideos();
}
