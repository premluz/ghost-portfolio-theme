...
async function initGradFlowBackground(canvas, configInput) {
  ...
  const program = new Program(gl, {
    ...
    uniforms: {
      ...
      u_wave_opacity: { value: config.waveOpacity != null ? config.waveOpacity : DEFAULT_CONFIG.waveOpacity },
  ...
  // Accumulated animation clock, advanced by a CLAMPED per-frame delta
  // rather than read straight off the wall clock.
  //
  // u_time used to be `(now - startTime) / 1000`. While the tab is
  // backgrounded (alt-tab to another app, another tab, a minimised window)
  // the browser throttles or fully stops rAF, but the wall clock keeps
  // running — so the first frame after returning jumped u_time forward by
  // the entire time away. The shader then advanced its whole noise field in
  // a single frame: a hard visual snap on return, and the GPU re-uploading a
  // completely different frame right as the compositor was already busy
  // restoring the tab.
  //
  // Clamping to ~100ms (about 3 frames at 60Hz) means a resume costs one
  // ordinary frame's worth of motion instead of however many seconds you
  // were away. The animation still runs off elapsed time for every normal
  // frame, so the original intent — "a band resumed after being scrolled
  // past picks up where it would have been" — is unchanged for the
  // IntersectionObserver start/stop below, which is measured in a frame or
  // two, not seconds.
  let animTime = 0;
  let prevNow = null;
  ...
  const tick = (now) => {
    ...
    animTime += Math.min(now - prevNow, MAX_FRAME_DELTA_MS);
    prevNow = now;
    program.uniforms.u_time.value = animTime / 1000;
    // Parallax progress for THIS canvas, measured against its own position
    // in the viewport: -1 when the element sits one viewport BELOW the fold,
    // 0 when it's centred, +1 when it's one viewport above. Multiplying that
    // by `parallax` gives a wave offset that tracks the element scrolling
    // past — which is what parallax means here.
    //
    // Was `window.scrollY / window.innerHeight` — ABSOLUTE page scroll, so
    // on a tall page (this homepage runs ~14 viewports) the value reaching a
    // band four viewports down was already ~4, not ~0. waveFlow() adds
    // `u_scroll * parallax` into the wave's y coordinate and then multiplies
    // by `taper = sin(uv.y * PI)`, which falls to zero at both band edges —
    // so that constant multi-viewport offset shoved the wave into its own
    // taper and FLATTENED it before any scrolling happened, worse the
    // further down the page the band sat. The old comment claimed "only the
    // RELATIVE change matters", which is only true if the baseline is
    // subtracted; nothing subtracted it.
    // getBoundingClientRect() per frame does cost layout, but only for
    // canvases that actually asked for parallax (needsScroll), and it is the
    // only way to know where THIS element is rather than where the page is.
    if (needsScroll) {
      const r = canvas.getBoundingClientRect();
      const centre = r.top + r.height / 2;
      program.uniforms.u_scroll.value =
        (window.innerHeight / 2 - centre) / window.innerHeight;
    }
    renderer.render({ scene });
    rafId = requestAnimationFrame(tick);
  };
  // prevNow is cleared on stop so the first frame after ANY restart seeds
  // itself (delta 0) instead of measuring against a timestamp from before
  // the pause — without this the clamp above would still take one oversized
  // step every time a band scrolls back into view.
  const start = () => { if (!rafId) rafId = requestAnimationFrame(tick); };
  const stop = () => { if (rafId) { cancelAnimationFrame(rafId); rafId = 0; prevNow = null; } };
