...
class ParticleMorphSystem {
  ...
  morphTo(state, duration = null) {
    ...
    if (nextState) {
      ...
      // Retry once the shape exists instead of dropping the request.
      //
      // GLB-derived shapes ('sphere', 'torus', …) are created asynchronously
      // (createInitialStates, after the meshes load — see initializeModules).
      // A visitor who scrolls quickly can reach a section whose shape hasn't
      // been registered yet; the request was simply discarded, and because
      // nothing ever re-issued it the layer stayed on whatever it was last
      // showing.
      //
      // That is the "scroll straight after landing and the hero's ground
      // plane stays on screen" bug: measured, 'hero-exit' correctly morphed
      // to 'terrain', then operating-model's morphTo('sphere') arrived with
      // registered:false and vanished — leaving 'terrain' visible, the shape
      // the exit is only meant to pass THROUGH.
      //
      // Guarded so only the LATEST pending request wins (a fast scroll can
      // queue several) and so it gives up rather than polling forever.
      this._pendingMorph = { state, duration };
      if (!this._pendingMorphTimer) {
        let tries = 0;
        this._pendingMorphTimer = setInterval(() => {
          const pending = this._pendingMorph;
          // Stop if the request was satisfied some other way, or we've waited
          // long enough that re-applying would fight the user's real position.
          if (!pending || ++tries > 40) {
            clearInterval(this._pendingMorphTimer);
            this._pendingMorphTimer = null;
            this._pendingMorph = null;
            return;
          }
          if (this.stateRegistry.get(pending.state)) {
            clearInterval(this._pendingMorphTimer);
            this._pendingMorphTimer = null;
            this._pendingMorph = null;
            this.morphTo(pending.state, pending.duration);
          }
        }, 100);
      }
    }
