...
// data-gradient-bands — which bands to render: "top" | "bottom" | "both"
// (default "both"; any unrecognised value falls back to "both"). A skipped
// band is never inserted into the DOM and never mounts its WebGL canvas, so
// it costs nothing — this is a real removal, not visibility:hidden.
...
// (posts-tabs-grid-lab.css insets its backdrop by the same value). Omit to
// keep the stylesheet's own 480px default.
// Was CSS-only, with no data- equivalent: every other knob on this
// component is a data-gradient-* attribute, so band height — arguably the
// most reached-for of them — was the one setting you couldn't find where
// you'd look for it. A stylesheet override still works and still wins where
// it's more specific; this is an addition, not a replacement.
// NOTE this is the size of the BOX the wave lives in, which is a different
// question from how tall the wave swings inside it (data-gradient-amplitude)
// or how thick the second wave's band is (data-gradient-wave2-width).
//
...
function readConfig(frame) {
  ...
  return {
    ...
    // PER-BAND parallax overrides. `parallax`/`wave2Parallax` above apply to
    // BOTH bands identically, so the top and bottom bands drift by the same
    // amount and never separate from each other. These four let each band
    // take its own value; null means "no override, use the shared one", so
    // every existing instance behaves exactly as before.
    // Resolved per band in bandConfig(), which already receives `position`.
    parallaxTop: num(d.gradientParallaxTop, null),
    ...
    waveOpacity: num(d.gradientWaveOpacity, DEFAULTS.waveOpacity),
    // A CSS length string ("700px", "40vh"), not a number — passed through
    // verbatim to a custom property rather than parsed, so any valid CSS
    // length unit works. null = don't set it, let the stylesheet default win.
    edgeHeight: d.gradientEdgeHeight || null,
    // Which bands to render: 'top' | 'bottom' | 'both'. Anything else (or
    // omitted) means 'both', which is the historical behaviour — so this is
    // purely additive and no existing instance changes.
    bands: (d.gradientBands === 'top' || d.gradientBands === 'bottom')
      ? d.gradientBands
      : 'both',
...
// Resolve a per-band override against the shared fallback. `top`/`bottom`
// are null when the instance didn't set data-gradient-*-top/-bottom, in
// which case both bands keep using `shared` — so adding these attributes is
// purely additive and every existing instance is untouched.
function pickBand(top, bottom, shared, isTop) {
  const own = isTop ? top : bottom;
...
function bandConfig(cfg, position, frame) {
  ...
  return Object.assign({
    ...
    noise: 0,
    resolutionScale: cfg.resolutionScale,
    // Per-band override wins over the shared value when supplied. This is
    // what lets the top and bottom bands drift at different rates instead
    // of moving as one — see the parallaxTop/Bottom doc in readConfig().
    parallax: pickBand(cfg.parallaxTop, cfg.parallaxBottom, cfg.parallax, outerAtTop),
    breathe: cfg.breathe,
    breatheRate: cfg.breatheRate,
    amplitude: cfg.amplitude,
    waveOpacity: cfg.waveOpacity,
    fadeOuter: cfg.fadeOuter,
    outerAtTop: outerAtTop,
    layer2: cfg.wave2Opacity > 0 ? {
      ...
      parallax: pickBand(cfg.wave2ParallaxTop, cfg.wave2ParallaxBottom, cfg.wave2Parallax, outerAtTop),
    } : null,
  }, edges);
}
...
function initFrame(frame) {
  ...
  // Set on the FRAME, not on each band: --gradient-frame-edge-height is a
  // custom property, so it inherits to both bands at once AND reaches
  // anything else scoped inside the frame that keys off it — notably
  // posts-tabs-grid-lab.css, which insets its backdrop by exactly this value
  // (top/bottom) so the backdrop never repaints over a band's own fade edge.
  // Setting it per-band would leave that inset reading the stale default.
  if (cfg.edgeHeight) {
    frame.style.setProperty('--gradient-frame-edge-height', cfg.edgeHeight);
  }
  ...
  // The height custom property is set on the FRAME (above) and reaches the
  // bands by inheritance — which only holds while they're inside it. On the
  // external-container path they are deliberately moved OUT of the frame's
  // subtree, so inheritance breaks and they would silently fall back to the
  // stylesheet's 480px. Mirror it onto that container too.
  // (No template uses data-gradient-bands-container today; this keeps the
  // escape hatch correct rather than leaving a trap for whoever needs it.)
  if (external && cfg.edgeHeight) {
    bandsContainer.style.setProperty('--gradient-frame-edge-height', cfg.edgeHeight);
  }
  ...
  // data-gradient-bands: only INSERT the requested band(s). Both entries in
  // `bands` are still created either way — several things below index it
  // positionally (bands[1] feeds bindShift's endInset), and a band that was
  // never inserted measures offsetHeight 0, which is exactly the right
  // answer for that inset when there IS no bottom band to compensate for.
  // Skipping insertion rather than hiding with CSS keeps the skipped band
  // out of layout entirely. It does NOT by itself stop the canvas mounting:
  // that loop iterates this array, not the DOM, so it carries its own
  // isConnected check — see the comment there.
  const showTop = cfg.bands === 'both' || cfg.bands === 'top';
  const showBottom = cfg.bands === 'both' || cfg.bands === 'bottom';
  if (showTop) bandsContainer.insertBefore(bands[0].band, bandsContainer.firstChild);
  if (showBottom) bandsContainer.appendChild(bands[1].band);
  applyColors();
  ...
  bands.forEach(function(entry) {
    // Skip bands that data-gradient-bands excluded. This loop walks the
    // ARRAY, not the DOM, so without this check a band that was never
    // inserted would still get its own canvas, WebGL context and rAF loop —
    // running a shader on a detached element nobody can see.
    if (!entry.band.isConnected) return;
