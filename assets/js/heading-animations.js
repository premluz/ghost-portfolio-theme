...
function initHeadingAnimations() {
  ...
  // Same rationale as card-scroll-reveal.js's window.__revealBackfill: an
  // instant scroll jump (curtain-return, browser scroll-restoration on
  // refresh landing already past a heading, any non-smooth programmatic
  // scroll) can move an element from "below viewport" to "above viewport"
  // between two frames without the IntersectionObserver ever seeing it
  // intersect — the reveal/reverse logic above never fires for that
  // crossing, and the heading is stuck at its hidden opacity:0 forever.
  // Mirrors that file's docTop/limit/viewportTop split: anything already
  // scrolled past (docTop < viewportTop) gets snapped instantly (nothing
  // to see happen), anything still on screen (viewportTop <= docTop <
  // limit) gets the real animated reveal so it doesn't just pop.
  window.__headingRevealBackfill = (maxDocY) => {
    ...
    let count = 0;
    candidates.forEach((heading) => {
      if (revealedHeadings.has(heading) || docTop(heading) >= limit) return;
      const mode = resolveMode(heading);
      const blurPx = resolveBlur(heading);
      if (docTop(heading) >= viewportTop) {
        if (mode === 'letter') _revealHeadingByLetter(heading, blurPx, noop);
        else if (mode === 'word') _revealHeadingByWord(heading, splitMap, wordTlMap, blurPx, noop);
        else _revealHeadingFade(heading, blurPx, noop);
      } else {
        gsap.killTweensOf(heading);
        if (mode === 'letter') {
          let letters;
          if (heading.dataset.lettersSplit === 'true') {
            ...
            _revealHeadingByLetter(heading, 0, noop);
            gsap.killTweensOf(heading);
            letters = Array.from(heading.querySelectorAll('.char'));
          }
          gsap.killTweensOf(letters);
          ...
          if (split && split.words && !split.fallback) {
            ...
            gsap.set(split.words, { opacity: 1, y: 0, clearProps: 'transform' });
            gsap.set(heading, { opacity: 1, filter: 'none' });
          } else {
            gsap.set(heading, { opacity: 1, filter: 'none' });
          }
