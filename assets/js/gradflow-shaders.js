...
const FRAGMENT = `
  ...
  // BASE-WAVE opacity. layer2 has always had its own (u_layer2_opacity);
  // the base ramp had none because it IS the band's opaque 4-colour ramp —
  // fading it needs something to fade TOWARD, which is the band's own edge
  // colour (u_color1/u_color4, whichever faces the page), not transparency:
  // the band element itself is what guarantees the seamless match with the
  // page background, so blending to alpha here would punch a hole in that
  // match rather than soften the wave. 1 = unchanged/opaque.
  uniform float u_wave_opacity;
  ...
  vec3 waveGradient(vec2 uv, float time) {
    ...
    // BASE-WAVE OPACITY. Blends the animated ramp back toward the band's
    // own FLAT edge ramp (the same u_color1->u_color4 gradient with no wave
    // displacement), not toward transparency — see u_wave_opacity's
    // declaration for why. At 1.0 this is a no-op and the branch above is
    // what shows; at 0 the band becomes a plain static vertical gradient.
    if (u_wave_opacity < 0.999) {
      float flatPattern = smoothstep(0.0, 1.0, clamp(uv.y, 0.0, 1.0));
      vec3 flatColor;
      if (flatPattern < 0.333) {
        flatColor = mix(u_color1, u_color2, smoothstep(0.0, 0.333, flatPattern));
      } else if (flatPattern < 0.667) {
        flatColor = mix(u_color2, u_color3, smoothstep(0.333, 0.667, flatPattern));
      } else {
        flatColor = mix(u_color3, u_color4, smoothstep(0.667, 1.0, flatPattern));
      }
      color = mix(flatColor, color, u_wave_opacity);
    }
...
const DEFAULT_CONFIG = {
  ...
  amplitude: 1,
  // Base-wave opacity — how much of the ANIMATED ramp shows versus the
  // band's own flat vertical gradient. 1 = fully animated (the historical
  // behaviour, so every existing instance is unchanged); 0 = a static
  // gradient with no wave motion at all. The layer2 counterpart is
  // wave2Opacity. See u_wave_opacity in the fragment shader for why this
  // blends toward the flat ramp rather than toward transparency.
  waveOpacity: 1,
  type: 'stripe',
  noise: 0.08,
